...
def makeKeyFiles(name, keySize):
    ...
    if os.path.exists(r'.\utils\%s_pubkey.txt' % (name)) or os.path.exists(r'.\utils\%s_privkey.txt' % (name)):
        sys.exit(r'WARNING: The file .\utils\%s_pubkey.txt or .\utils\%s_privkey.txt already exists! Use a different name or delete these files and re-run this program.' % (name, name))
    ...
    fo = open('./utils/%s_pubkey.txt' % (name), 'w')
    ...
    fo = open('./utils/%s_privkey.txt' % (name), 'w')
